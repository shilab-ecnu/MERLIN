---
title: 'Part 2: Model Fitting Using Simulated Data'
date: "2026-08-24"
output:
  html_document:
    toc: true
...
vignette: |
  ...
  %\VignetteIndexEntry{Part 2: Model Fitting Using Simulated Data}
...
interacts with genetic variants $G$ to influence both the exposure $X$
and the outcome $Y$.
...
$$
Y=(\beta_1+\beta_4E)X+G\beta_2+E\beta_E+\epsilon_Y.
$$ Where: 
...
-   $\gamma_3$: The genetic-by-environment interaction effects on the
    exposure, corresponding to $\boldsymbol{\gamma}^{(GI)}$
    in the manuscript.
...
-   $\gamma_E$: The effect of environment E on the
    exposure, corresponding to $\boldsymbol{\gamma}^{(E)}$
    in the manuscript.
...
effects ($\gamma_3$) are generated from a bivariate normal distribution
to allow for correlation (`cor_g1g3 = 0.4`) between them. We control
their variance using specified heritability parameters (`h_g1` and
`h_g3`).
