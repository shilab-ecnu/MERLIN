...
#include <thread>
#include <mutex>
#include "Gibbs_samp_Gam3seo_binary.h"

using namespace Rcpp;
using namespace arma;
using namespace std;

//This is for different proportion between male and female
...
ObjGibbs_Gam3seo_binary MRGibbs_Gam3seo_binary(arma::vec &gammah1, arma::vec &gammah3, arma::vec&Gammah1, arma::vec&Gammah3,
                          arma::mat &invS1, arma::mat &invS2, arma::mat &invS3, arma::mat &invS4, arma::mat &S4,
                          arma::mat &R, arma::mat &invR, double &rho_1, double &rho_2, double &mup, int maxIter, int burnin, int thin)
{
...
//Rprintf("Have a look of beta1 = %4f \n",mup);
...
mat s3rs3s4s2 = invS3 * R * invS3 * S4 * invS2;
mat s3rs3s4rs4s3rs3 = invS3 * R * invS3 * S4 * invR * S4 * invS3 * R * invS3;
...
vec s3rs3s4s2_temp = diagvec(s3rs3s4s2);
vec s3rs3s4rs4s3rs3_temp = diagvec(s3rs3s4rs4s3rs3);
...
vec s3rs3s4rs2g3_hat = invS3 * R * invS3 * S4 * invR * invS2 * gammah3;
vec s3rs3s4rs4G3_hat = invS3 * R * invS3 * S4 * invR * invS4 * Gammah3;
...
//-----------------------------------Begin to give some middle value-----------
   ...
   //Rprintf("Have a look of G1tu3 = %4f \n",G1tu3);
    mu1 =  rho1 * (- (dot(s1rs1.row(j), gamma1) - gamma1[j] * s1rs1_temp[j] )
          ...
          +rho2 * (- b42 * (dot(s4rs4.row(j), gamma1) - gamma1[j] * s4rs4_temp[j] )
                   ...
                   - mup * b42 * dot(s3rs3.row(j), gamma3)
                  ) * v12[j] ;
 ...
 for(int j = 0; j < p; j++){
    ...
    v32[j] = 1. / (b42 * rho1 * s3rs3_temp[j] + invsg32
                  ...
                  - 2 * rho_2 * rho2 * beta4 * mup * s3rs3s4s2_temp[j] 
                  + 2 * rho2 * mup * beta1 * beta4 * s3rs3_temp[j]
                  + rho2 * mup * mup * b42 * s3rs3s4rs4s3rs3_temp[j]
                   ) ;     
    double mu3;
    mu3 = rho1 * (- b42 * (dot(s3rs3.row(j), gamma3) - gamma3[j] * s3rs3_temp[j] )
         ...
         +rho2 * (- (dot(s2rs2.row(j), gamma3) - gamma3[j] * s2rs2_temp[j] )
                   ...
                   - rho_2 * beta4 * mup * s3rs3s4rs2g3_hat[j] 
                   + mup * beta4 * s3rs3s4rs4G3_hat[j]
                   - mup * b42 * dot(s3rs3.row(j), gamma1)
                   + 2 * rho_2 * beta4 * mup * (dot(s3rs3s4s2.row(j), gamma3) - gamma3[j] * s3rs3s4s2_temp[j] )
                   - 2 * mup * beta1 * beta4 * (dot(s3rs3.row(j), gamma3) - gamma3[j] * s3rs3_temp[j] )
                   - mup * mup * b42 * (dot(s3rs3s4rs4s3rs3.row(j), gamma3) - gamma3[j] * s3rs3s4rs4s3rs3_temp[j] )
                 ) * v32[j];
    mu33[j] = mu3 + randn()*sqrt(v32[j]); 
    //Rprintf("Have a look of gamma3 = %4f \n", gamma3[j] * s3rs3_temp[j]  );
    gamma3[j] = mu33[j];
    //gamma3[j] = 0 * gamma3[j];
 ...
 for(int j = 0; j < p; j++){
    ...
    mub1 = rho1 * as_scalar(-rho_1 * gamma1.t() * s1s3g1_hat + gamma1.t() * s3s3G1_hat
          ...
          +rho2 * as_scalar(-rho_2 * gamma3.t() * s2s4g3_hat + gamma3.t() * s4s4G3_hat
                            ...
                            - mup * beta4 * gamma3.t() * s3rs3 * gamma3
                           )* vb12;
    ...
    vb42 = 1. / as_scalar( rho1 * gamma3.t() * s3rs3 * gamma3 +  rho2 * gamma1.t() * s4rs4 * gamma1
                         ...
                         + 2 * rho2 * mup * gamma3.t() * s3rs3 * gamma1
                         + rho2 * mup * mup * gamma3.t() * s3rs3s4rs4s3rs3 * gamma3
                           );
    double mub4;
    mub4 = rho1 * as_scalar(-rho_1* gamma3.t() * s1s3g1_hat + gamma3.t() * s3s3G1_hat
          ...
          +rho2 * as_scalar(-rho_2 * gamma1.t() * s2s4g3_hat + gamma1.t() * s4s4G3_hat
                            ...
                            - rho_2 * mup * gamma3.t() * s3rs3s4rs2g3_hat 
                            + rho_2 * mup * gamma3.t() * s3rs3s4s2 * gamma3
                            + mup * gamma3.t() * s3rs3s4rs4G3_hat 
                            - mup * beta1 * gamma3.t() * s3rs3 * gamma3
                           ) * vb42 ;
    mub44 = mub4 + randn()*sqrt(vb42); 
    beta4 = mub44;
    
    //Rprintf("Have a look of beta4 = %4f \n",beta4); 
    //beta4 = 0.0;  
    //update sigma123------------------------------------------------------------
    sig12 = Updatesig2(gamma1, ak1, bk1, p );
    sigma12 = sig12;
    //Rcpp::Rcout << "gammah1 length: " << gammah1.n_elem << std::endl;
    //Rcpp::Rcout << "Value of p: " << p << std::endl;

    //Rprintf("Have a look of pmup = %4f \n",p);
    //sigma12 = sg1;
...
//[[Rcpp::export]]
List MRGEI_Gam3seo_binary(arma::vec &gammah1,arma::vec &gammah3, arma::vec &Gammah1, arma::vec &Gammah3,
                   arma::vec &se1, arma::vec &se2, arma::vec &se3, arma::vec &se4, arma::mat &R, double &rho_1, double &rho_2, double &p1, int maxIter, int burnin, int thin)
{
    ...
    mat invR = inv(R);
    mat S4 = diagmat(se4);
    double mup = (1 - 2*p1)/sqrt(p1 - p1*p1);

    
    ObjGibbs_Gam3seo_binary obj = MRGibbs_Gam3seo_binary( gammah1, gammah3, Gammah1, Gammah3, invS1, invS2, invS3, invS4, S4, R, invR, rho_1, rho_2, mup, maxIter, burnin, thin);
 ...
 // ObjGibbs3 obj = MRcorrObj(gammah, Gammah, se1, se2, lp_opt);
  List output = List::create(
    ...
    Rcpp::Named("S1RS3") = Rcpp::wrap(obj.s3rs3_temp)
                   ...
                   double &rho_1, double &rho_2, double &p1, int maxIter, int burnin, int thin)
{
    ...
    mat invR = inv(R);
    mat S4 = diagmat(se4);
    double mup = (1 - 2*p1)/(p1 - p1*p1);

    
    ObjGibbs_Gam3seo_binary obj = MRGibbs_Gam3seo_binary( gammah1, gammah3, Gammah1, Gammah3, invS1, invS2, invS3, invS4, 
                                                          S4, R, invR, rho_1, rho_2, mup, maxIter, burnin, thin);
