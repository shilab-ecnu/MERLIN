...
#include <thread>
#include <mutex>
#include "Gibbs_samp_dropg3seo.h"
...
ObjGibbs_dropg3 MRGibbs_dropg3(arma::vec &gammah1, arma::vec&Gammah1, arma::vec&Gammah3,
                          arma::mat &invS1, arma::mat &invS3, arma::mat &invS4,
                          arma::mat &R, double &rho_1, int maxIter, int burnin, int thin)
{
    ...
    //------------Begin to give some middle value-----------
        ...
        for(int j = 0; j < p; j++){
            ...
            mu1 =  rho1 * (- (dot(s1rs1.row(j), gamma1) - gamma1[j] * s1rs1_temp[j] )
                   ...
                   - beta1 * dot(s3rs3.row(j), beta2) - beta1 * beta4 * dot(s3rs3.row(j), gamma3)
                    ...
                    +(- b42 * (dot(s4rs4.row(j), gamma1) - gamma1[j] * s4rs4_temp[j] )
                   + beta4 * s4s4G3_hat[j]
                   - beta1 * beta4 * dot(s4rs4.row(j), gamma3)
                  ) * v12[j] ;
        ...
        for(int j = 0; j < p; j++){
            ...
            +(     - b12 * (dot(s4rs4.row(j), gamma3) - gamma3[j] * s4rs4_temp[j] )
                   + beta1 * s4s4G3_hat[j]
                   - beta1 * beta4 * dot(s4rs4.row(j), gamma1)
                 ) * v32[j];
        ...
        //update beta1
        vb12 = 1. / as_scalar( rho1 * gamma1.t() * s3rs3 * gamma1 + gamma3.t() * s4rs4 * gamma3);
        double mub1;
        mub1 = rho1 * as_scalar(-rho_1 * gamma1.t() * s1s3g1_hat + gamma1.t() * s3s3G1_hat
               ...
               + as_scalar(gamma3.t() * s4s4G3_hat
                            - beta4 * gamma1.t() * s4rs4 * gamma3
                           )* vb12;
        mub11 = mub1 + randn()*sqrt(vb12);
        beta1 = mub11;  
        //update beta4
        vb42 = 1. / as_scalar( rho1 * gamma3.t() * s3rs3 * gamma3 + gamma1.t() * s4rs4 * gamma1);
        double mub4;
        mub4 = rho1 * as_scalar(-rho_1* gamma3.t() * s1s3g1_hat + gamma3.t() * s3s3G1_hat
               ...
               + as_scalar(gamma1.t() * s4s4G3_hat
                            - beta1 * gamma1.t() * s4rs4 * gamma3 
                           ) * vb42 ;
...
//[[Rcpp::export]]
List MRGEI_dropg3(arma::vec &gammah1, arma::vec &Gammah1, arma::vec &Gammah3,
                   ...
                   int maxIter, int burnin, int thin)
{
    ...
    ObjGibbs_dropg3 obj = MRGibbs_dropg3(gammah1, Gammah1, Gammah3, invS1, invS3, invS4, R, rho_1, maxIter, burnin, thin);
