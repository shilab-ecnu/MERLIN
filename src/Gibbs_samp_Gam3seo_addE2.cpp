...
#include <thread>
#include <mutex>
#include "Gibbs_samp_Gam3seo_addE2.h"
...
ObjGibbs_Gam3seo_addE2 MRGibbs_Gam3seo_addE2(arma::vec &gammah1, arma::vec &gammah3, arma::vec&Gammah1, arma::vec&Gammah3,
                          ...
                          arma::mat &R, double &rho_1, double &rho_2, int maxIter, int burnin, int thin)
{
...
mat invS34 = invS4 * R * invS4 * inv(invS3) * inv(R);

mat s1rs1 = invS1 * R * invS1;
mat s3rs3 = invS3 * R * invS3;
mat s1rs3 = invS1 * R * invS3;

mat s1rs34 = invS1 * R * invS34;
mat s3rs34 = invS3 * R * invS34;
mat s34rs34 = invS34 * R * invS34;
...
vec s34rs34_temp = diagvec(s34rs34);
...
vec s1s34g1_hat = invS1 * invS34 * gammah1;
vec s3s34G1_hat = invS3 * invS34 * Gammah1;
...
//-----------------------------------Begin to give some middle value-----------
 ...
 for(int j = 0; j < p; j++){
   ...
   //Rprintf("Have a look of G1tu3 = %4f \n",G1tu3);
    mu1 =  rho1 * (- (dot(s1rs1.row(j), gamma1) - gamma1[j] * s1rs1_temp[j] )
                   ...
                   + rho_1 * dot(s1rs3.row(j), beta2) + rho_1 * beta4 * dot(s1rs34.row(j), gamma3)
                   - beta1 * dot(s3rs3.row(j), beta2) - beta1 * beta4 * dot(s3rs34.row(j), gamma3)
                  ) * v12[j]
 ...
 for(int j = 0; j < p; j++){
      //update  gamma3-----------------------------------------------------
    v32[j] = 1. / (b42 * rho1 * s34rs34_temp[j] + invsg32
                  + rho2 * s2rs2_temp[j] - 2 * rho_2 * rho2 * beta1 * s2rs4_temp[j] + rho2 * b12 * s4rs4_temp[j] ) ;     
    double mu3;
    mu3 = rho1 * (- b42 * (dot(s34rs34.row(j), gamma3) - gamma3[j] * s34rs34_temp[j] )
                  - rho_1 * beta4 * s1s34g1_hat[j] + beta4 * s3s34G1_hat[j]
                  + rho_1 * beta4 * dot(s1rs34.row(j), gamma1) - beta1 * beta4 * dot(s3rs34.row(j), gamma1) - beta4 * dot(s3rs34.row(j), beta2)
                 ) * v32[j]
 ...
 for(int j = 0; j < p; j++){
    ...
    mub2 = rho1 * (- (dot(s3rs3.row(j), beta2) - beta2[j] * s3rs3_temp[j] )
                   ...
                   + rho_1 * dot(s1rs3.row(j), gamma1) - beta1 * dot(s3rs3.row(j), gamma1) - beta4 * dot(s3rs34.row(j), gamma3)
                  ) * vb22[j];
    ...
    mub1 = rho1 * as_scalar(-rho_1 * gamma1.t() * s1s3g1_hat + gamma1.t() * s3s3G1_hat
                            + rho_1 * gamma1.t() * s1rs3 * gamma1 - gamma1.t() * s3rs3 * beta2 - beta4 * gamma1.t() * s3rs34 * gamma3
                           )* vb12
    ...
    //update beta4-----------------------------------------------------------
    vb42 = 1. / as_scalar(rho1 * gamma3.t() * s34rs34 * gamma3 +  rho2 * gamma1.t() * s4rs4 * gamma1);
    //Rprintf("Have a look of vb12 = %4f \n",vb12);
    double mub4;
    mub4 = rho1 * as_scalar(-rho_1* gamma3.t() * s1s34g1_hat + gamma3.t() * s3s34G1_hat
                            + rho_1 * gamma3.t() * s1rs34 * gamma1 - beta1 * gamma1.t() * s3rs34 * gamma3 - beta2.t() * s3rs34 * gamma3
                           ) * vb42
...
//[[Rcpp::export]]
List MRGEI_Gam3seo_addE2(arma::vec &gammah1,arma::vec &gammah3, arma::vec &Gammah1, arma::vec &Gammah3,
                   ...
                   double &rho_1, double &rho_2, int maxIter, int burnin, int thin)
{
    ...
    ObjGibbs_Gam3seo_addE2 obj = MRGibbs_Gam3seo_addE2( gammah1, gammah3, Gammah1, Gammah3, invS1, invS2, invS3, invS4, 
                                                        R, rho_1, rho_2, maxIter, burnin, thin);
