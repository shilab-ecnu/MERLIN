...
#include <thread>
#include <mutex>
#include "Gibbs_samp_Gam3seo_3to1.h"
...
ObjGibbs_Gam3seo_3to1 MRGibbs_Gam3seo_3to1(arma::vec &gammah1, arma::vec &gammah3, arma::vec&Gammah1, arma::vec&Gammah3, 
                                               arma::mat &invS1, arma::mat &invS2, arma::mat &invS3, arma::mat &invS4, 
                                               arma::mat &R,
                                               double &rho_1, double &rho_2, double &mup, 
...
vec s1rs3_sum_temp = diagvec(s1rs3_sum);
vec s2rs4_sum_temp = diagvec(s2rs4_sum);
...
//-----------------------------------Begin to give some middle value-----------
  ...
  double beta1_mu = beta1 + mup * beta4;
  double beta1_mu2 = beta1_mu * beta1_mu;
 ...
 for(int j = 0; j < p; j++){
    ...
    mu1 =  rho1 * (- (dot(s1rs1.row(j), gamma1) - gamma1[j] * s1rs1_temp[j] )
                   + rho_1 * beta1 * (dot(s1rs3_sum.row(j), gamma1) - gamma1[j] * s1rs3_sum_temp[j])
          ...
          +rho2 * (- b42 * (dot(s4rs4.row(j), gamma1) - gamma1[j] * s4rs4_temp[j] )
                   ...
                   - beta1 * beta4 * dot(s4rs4.row(j), gamma3) + rho_2 * beta4 * dot(s4rs2.row(j), gamma3)
                   - mup * b42 * dot(s4rs4.row(j), gamma3)
                  ) * v12[j] ;
 ...
 for(int j = 0; j < p; j++){
      //update  gamma3-----------------------------------------------------
    v32[j] = 1. / (b42 * rho1 * s3rs3_temp[j] + invsg32 + rho2 * s2rs2_temp[j] - 2 * rho_2 * rho2 * beta1_mu * s2rs4_temp[j] + rho2 * beta1_mu2 * s4rs4_temp[j]);    
    double mu3;
    mu3 = rho1 * (- b42 * (dot(s3rs3.row(j), gamma3) - gamma3[j] * s3rs3_temp[j] )
                  - rho_1 * beta4 * s1s3g1_hat[j] + beta4 * s3s3G1_hat[j]
                  + rho_1 * beta4 * dot(s3rs1.row(j), gamma1) - beta1 * beta4 * dot(s3rs3.row(j), gamma1) - beta4 * dot(s3rs3.row(j), beta2)
                 ) * v32[j]
         + rho2 * (- (dot(s2rs2.row(j), gamma3)- gamma3[j] * s2rs2_temp[j])
                   + rho_2 * beta1_mu * (dot(s2rs4_sum.row(j), gamma3) - gamma3[j] * s2rs4_sum_temp[j])
                   - beta1_mu2 * (dot(s4rs4.row(j), gamma3) - gamma3[j] * s4rs4_temp[j])
                   - rho_2 * beta1_mu * s2s4g3_hat[j] + beta1_mu * s4s4G3_hat[j]
                   + s2s2g3_hat[j] - rho_2 * s2s4G3_hat[j]
                   + rho_2 * beta4 * dot(s2rs4.row(j), gamma1) - beta1_mu * beta4 * dot(s4rs4.row(j), gamma1)
                   ) * v32[j];
 ...
 for(int j = 0; j < p; j++){
    ...
    mub2 = rho1 * (- (dot(s3rs3.row(j), beta2) - beta2[j] * s3rs3_temp[j] )
                   ...
                   + rho_1 * dot(s3rs1.row(j), gamma1) - beta1 * dot(s3rs3.row(j), gamma1) - beta4 * dot(s3rs3.row(j), gamma3)
                  ) * vb22[j];
    ...
    mub1 = rho1 * as_scalar(-rho_1 * gamma1.t() * s1s3g1_hat + gamma1.t() * s3s3G1_hat
          ...
          +rho2 * as_scalar(-rho_2 * gamma3.t() * s2s4g3_hat + gamma3.t() * s4s4G3_hat
                            ...
                            - mup * beta4 * gamma3.t() * s4rs4 * gamma3
                           )* vb12;
    ...
    //update beta4-----------------------------------------------------------
    vec gamma1_mu = gamma1 + mup * gamma3;
    vb42 = 1. / as_scalar(rho1 * gamma3.t() * s3rs3 * gamma3 + rho2 * gamma1_mu.t() * s4rs4 * gamma1_mu);
    double mub4;
    mub4 = rho1 * as_scalar(-rho_1* gamma3.t() * s1s3g1_hat + gamma3.t() * s3s3G1_hat
                            + rho_1 * gamma3.t() * s3rs1 * gamma1 - beta1 * gamma1.t() * s3rs3 * gamma3 - beta2.t() * s3rs3 * gamma3
                           ) * vb42
         + rho2 * as_scalar(- rho_2 * gamma1_mu.t() * s2s4g3_hat
                            + gamma1_mu.t() * s4s4G3_hat
                            + rho_2 * gamma1_mu.t() * s4rs2 * gamma3
                            - beta1 * gamma1_mu.t() * s4rs4 * gamma3
                            ) * vb42;
...
//[[Rcpp::export]]
List MRGEI_Gam3seo_3to1(arma::vec &gammah1,arma::vec &gammah3, arma::vec &Gammah1, arma::vec &Gammah3,
                   ...
                   double &rho_1, double &rho_2, double &p1, int maxIter, int burnin, int thin)
{
    ...
    ObjGibbs_Gam3seo_3to1 obj = MRGibbs_Gam3seo_3to1( gammah1, gammah3, Gammah1, Gammah3, invS1, invS2, invS3, invS4, R, rho_1, rho_2, mup, maxIter, burnin, thin);
