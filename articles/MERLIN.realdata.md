...
- `"discrete_E_adj"`: An adjusted version of the discrete environmental
  ...
  requires `p_exp` and `p_out`, representing P(E=1) in the exposure and
  outcome samples, respectively. Before model fitting, the exposure GWAS
...
and females (based on `MERLINpvalue4`). A significant \beta_4 implies a
strong environment-dependent (sex-specific) causal pathway.
