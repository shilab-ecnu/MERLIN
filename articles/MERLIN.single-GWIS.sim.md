...
effects (\gamma_3) are simulated from a bivariate normal distribution.
Their variances are controlled by `h_g1` and `h_g3`, and `cor_g1g3`
determines the correlation between the two sets of genetic effects.
...
X and Y are correlated to represent unmeasured confounding, while the
exposure and outcome summary statistics are calculated in
...
instrument-selection threshold. In real applications, genome-wide
...
\widehat{\Gamma}\_3 and its standard error are omitted. MERLIN-MO uses
the exposure GWAS, exposure GWIS, and outcome GWAS summary statistics to
jointly estimate the average causal effect \beta_1 and the interaction
effect \beta_4 within the reduced-data model. Although the outcome GWIS
is unavailable, uncertainty in both causal parameters is retained during
model fitting.
...
associations on the exposure-GWAS associations. It estimates \beta_4 by
a second MR-Egger regression of the outcome-GWIS associations on the
