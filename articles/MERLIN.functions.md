...
EstRhofun(fileexposure, fileoutcome, stringname3, ld_r2_thresh, pth, plink_dir)
```
...
| Argument | Description |
|----|----|
| `fileexposure` | Path to exposure summary statistics. The file must contain `SNP`, `BETA`, and `SE` columns. |
| `fileoutcome` | Path to outcome summary statistics. The file must contain `SNP`, `BETA`, and `SE` columns. |
| `stringname3` | Prefix of the PLINK reference panel files, without the `.bed`, `.bim`, or `.fam` extension. |
| `ld_r2_thresh` | LD threshold passed to PLINK `--indep-pairwise` when selecting approximately independent variants. |
| `pth` | Absolute Z-score threshold used to identify null variants in both summary-statistics datasets. |
| `plink_dir` | Path to the PLINK executable. |

##### Value

A list containing `rhohat`, the estimated sample-overlap correlation;
`pvalue`, the correlation test P-value; `pres`, the number of
independent null variants used; and `Rhores`, saved samples from the
truncated-normal correlation estimator.
