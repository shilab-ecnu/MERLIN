...
`MRGEI_dropg3()`  
Run the `drop_g3`(without exposure gwis) MERLIN MCMC kernel.
...
| `gammah3` | Numeric vector of exposure GWIS interaction-effect estimates. Required for `standard`, `continuous_E`, `binary`, and `drop_G3`. |
| `Gammah1` | Numeric vector of outcome GWAS main-effect estimates. |
| `Gammah3` | Numeric vector of outcome GWIS interaction-effect estimates. Required for `standard`, `continuous_E`, `binary`, and `drop_g3`. |
...
| `b1` | The main effect estimate ($`\beta_1`$), which can be either manually specified or obtained from an alternative MR method. When the “`drop_G3`” mode is selected, the Egger method is used to estimate $`\beta_1`$. |
| `maxIter`, `burnin`, `thin` | MCMC control parameters. |
